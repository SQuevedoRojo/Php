<?php
    ...
    function saberNomina($empleado)
    {
        try
        {
            ...
            $stmt = $GLOBALS["conn"]->prepare("SELECT salary from salaries s where s.emp_no = :empleado order by to_date asc limit 1");
            $stmt->bindParam(':empleado', $empleado);
            $stmt -> execute();
            $stmt->setFetchMode(PDO::FETCH_ASSOC);
            $salarios = $stmt->fetchAll();
            $stmt = $GLOBALS["conn"]->prepare("SELECT title from titles t where t.emp_no = :empleado order by to_date asc desc limit 1");
            $stmt->bindParam(':empleado', $empleado);
            $stmt -> execute();
            $stmt->setFetchMode(PDO::FETCH_ASSOC);
